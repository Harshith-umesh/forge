...
class PostProcessingPlugin(ABC):
    ...
    def export_dashboard_csv(self, model: UnifiedRunModel, output_path: Path) -> str:
        ...
        """
        # Base implementation fails explicitly to prevent empty CSV files
        # Plugins that support dashboard CSV export must override this method
        raise NotImplementedError(
            f"Plugin {self.__class__.__name__} does not implement dashboard CSV export. "
            f"Override export_dashboard_csv() to provide dashboard KPI export functionality."
        )
