...
def test_render_guidellm_jobs_set_host_users_only_when_requested() -> None:
    default_job = render_guidellm_job_from_parts(**_render_args())
    b200_job = render_guidellm_job_from_parts(**_render_args(), host_users=False)
    default_shared_job = render_guidellm_shared_volume_job_from_parts(**_render_args())
    b200_shared_job = render_guidellm_shared_volume_job_from_parts(
        **_render_args(), host_users=False
    )

    assert "hostUsers" not in default_job["spec"]["template"]["spec"]
    assert b200_job["spec"]["template"]["spec"]["hostUsers"] is False
    assert "hostUsers" not in default_shared_job["spec"]["template"]["spec"]
    assert b200_shared_job["spec"]["template"]["spec"]["hostUsers"] is False
