...
def init(allow_no_config_file=False):
    ...
    if not config_file:
        logging.info(
            "Running without TOPSAIL_FROM_CONFIG_FILE, skipping most of the initialization"
        )
        return

    config.init(pathlib.Path(config_file).parent)

    is_plot_test = config.project.get_config("PR_POSITIONAL_ARG_0", "", warn=False).endswith(
        "-plot"
    )
    if is_plot_test:
    ...
    if not matbench_preset:
        ...
        config.project.set_config(
            "matbench.download.url_file", workload_storage_dir / "data" / f"{matbench_preset}.yaml"
        )

    matbench_config = config.Config(
        workload_storage_dir,
        workload_storage_dir / "data" / config.project.get_config("matbench.config_file"),
    )
...
def generate_visualizations(results_dirname, generate_lts=None, test_failed=None):
    ...
    for idx in range(len(visualizations)):
        generate_visualization(
            results_dirname, idx, generate_lts=generate_lts, test_failed=test_failed
        )
...
def call_parse(step_idx, common_args, common_env):
    ...
    cmd = f"{parse_env_str} /app/forge/projects/matrix_benchmarking/subproject/bin/matbench parse {parse_args_str} |& tee > {log_file}"
...
def call_generate_lts(step_idx, common_args, common_env_str):
    ...
    cmd = f"{common_env_str} /app/forge/projects/matrix_benchmarking/subproject/bin/matbench parse {lts_args_str} |& tee > {log_file}"
...
def call_generate_lts_schema(step_idx, common_args):
    ...
    cmd = f"/app/forge/projects/matrix_benchmarking/subproject/bin/matbench generate_lts_schema {lts_schema_args_str} |& tee > {log_file}"
...
def call_download_lts(step_idx, common_args, common_env_str):
    ...
    cmd = f"{common_env_str} /app/forge/projects/matrix_benchmarking/subproject/bin/matbench download_lts {download_args_str} |& tee > {log_file}"
...
def call_upload_lts(step_idx, common_args, common_env_str):
    ...
    cmd = f"{common_env_str} /app/forge/projects/matrix_benchmarking/subproject/bin/matbench upload_lts {upload_args_str} |& tee > {log_file}"
...
def call_analyze_lts(step_idx, common_args, common_env_str):
    ...
    cmd = f"{common_env_str} /app/forge/projects/matrix_benchmarking/subproject/bin/matbench analyze_lts {analyze_args_str} |& tee > {log_file}"
...
def call_visualize(step_idx, common_env_str, common_args, filters_to_apply, generate_url):
    ...
    cmd = f"{common_env_str} /app/forge/projects/matrix_benchmarking/subproject/bin/matbench visualize {visu_args_str} |& tee > {log_file}"
...
def generate_visualization(
    results_dirname, idx, generate_lts=None, upload_lts=None, analyze_lts=None, test_failed=None
):
    ...
    do_analyze_lts = (
        analyze_lts
        if analyze_lts is not None
        else config.project.get_config("matbench.lts.regression_analyses.enabled", None)
    )

    if replotting and not config.project.get_config(
        "matbench.lts.regression_analyses.enabled_on_replot", None
    ):
        do_analyze_lts = False

    do_upload_lts = (
        upload_lts
        if upload_lts is not None
        else config.project.get_config("matbench.lts.opensearch.export.enabled", None)
    )

    if replotting and not config.project.get_config(
        "matbench.lts.opensearch.export.enabled_on_replot", None
    ):
        do_upload_lts = False
    ...
    if do_analyze_lts or do_upload_lts:
        try:
            ...
            fail_on_regression = config.project.get_config(
                "matbench.lts.regression_analyses.fail_test_on_regression"
            )
            fail_on_upload = config.project.get_config(
                "matbench.lts.opensearch.export.fail_test_on_fail"
            )
    ...
    if do_analyze_lts:
        ...
        step_idx += 1
        analyze_lts_errors, regression_detected = call_analyze_lts(
            step_idx, common_args, common_env_str
        )
        ...
        if regression_detected:
            if config.project.get_config(
                ...
                logging.warning(
                    "A regression has been detected, but ignored as per matbench.lts.regression_analyses.fail_test_on_regression"
                )
    ...
    if do_upload_lts:
        if config.project.get_config(
            "matbench.lts.regression_analyses.upload_lts_on_regression", True
        ):
            skip_upload = (
                non_fatal_errors
        ...
        if not skip_upload:
            ...
            if config.project.get_config("matbench.lts.opensearch.export.fail_test_on_fail"):
                ...
                logging.warning(
                    "An error happened during the LTS upload. Ignoring as per matbench.lts.opensearch.export.fail_test_on_fail."
                )
        else:
            logging.error(
                "Errors have been detected in the post-processing. NOT uploading LTS payload to OpenSearch."
            )
    ...
    for filters_to_apply in filters:
        ...
        step_idx += 1
        non_fatal_errors += call_visualize(
            step_idx, common_env_str, common_args, filters_to_apply, generate_url
        )
...
def download(results_dirname):
    ...
    if not env_key or not secret_path or not secret_filename:
        logging.warning(
            f"secrets.dir.env_key or ${secret_path} not set or {secret_filename=}. Cannot set AWS_SHARED_CREDENTIALS_FILE"
        )
    else:
    ...
    run.run(
        f"{common_env_str} /app/forge/projects/matrix_benchmarking/subproject/bin/matbench download {download_args_str} |& tee > {env.ARTIFACT_DIR}/_matbench_download.log"
    )


# generates a unique (sequential) build identifier
#
# uses the OpenSearch configuration to store the build counter
#
# `build_identifier` should be a fully qualified name, like
# `topsail-v1.2.0`, so that the build index starts at 1, as expected
def get_next_build_index(build_identifier):
    import opensearchpy
    import opensearchpy.exceptions

    # warning: `env_doc` contains the OpenSearch secrets, don't print it
    env_doc = generate_opensearch_config_yaml_env()

    client = opensearchpy.OpenSearch(
        hosts=[{"host": env_doc["opensearch_host"], "port": env_doc["opensearch_port"]}],
        timeout=60,
        http_compress=True,
        http_auth=(env_doc["opensearch_username"], env_doc["opensearch_password"]),
        use_ssl=True,
        verify_certs=False,
    ...
    _index = config.project.get_config("matbench.lts.opensearch.build_counter_index")
    index_prefix = config.project.get_config("matbench.lts.opensearch.index_prefix") or ""
    index = f"{index_prefix}{_index}"
    ...
    try:
        ...
        logging.error(
            ...
            f"Configured to use the '{env_doc['instance']}' instance."
        )
        raise
    ...
    return opensearch_increment_and_get_counter()
