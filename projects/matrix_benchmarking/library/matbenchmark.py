...
def save_benchmark_file(benchmark_file_content, dirname=None, name="benchmark.yaml"):
    ...
    benchmark_file_safe_content = json.loads(
        json.dumps(benchmark_file_content, default=functools.partial(_json_dumper, strict=False))
    )

    yaml_content = yaml.dump(
        benchmark_file_safe_content, default_flow_style=False, sort_keys=False, width=1000
    )
...
def run_benchmark(args, dry_run=False):
    ...
    try:
        ...
        logging.error(
            f"MatrixBenchmark benchmark rehearsal failed. See '{rehearsal_log_file}' for further detals."
        )
        return True  # failed
